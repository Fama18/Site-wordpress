=== Catmandu ===
Contributors: codemanas
Requires at least: WordPress 5.1
Tested up to: WordPress 5.4
Stable tag: 1.0.6
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

== Description ==

Catmandu is clean, responsive, modern and feature rich multi-purpose Business WordPress theme. It is suitable for business, portfolio, digital agencies and general corporate website. It is designed with great attention to details, flexibility and performance thus looks stunning on all types of screens and devices. Catmandu comes with creative and well-organized sections and very extensible and secure code. It is designed and developed paying careful attention to detail, to bring you a clean, modern and crisp design with the kinds of useful web elements that help make your website stand out from the crowd.

== Changelog ==

= 1.0.6 - Jul 5, 2020 =
* Fix sidr menu warning

= 1.0.5 - Jul 5, 2020 =
* Update theme URI

= 1.0.4 - Jul 3, 2020 =
* Fix: Warnings on top header

= 1.0.3 - Jul 1, 2020 =
* Update escping function on Go Top attribute

= 1.0.2 - Jun 27, 2020 =
* Delete: rtl.css
* Delete: Default values on home sections and other
* Fix: Sub menu accessibility issue
* Fix: Javascript console error
* Update: Replace wp_reset_query with wp_reset_postdata
* Fix: Escaping and sanitization
* Update: Responsive design issues

= 1.0.1 - May 29, 2020 =
* Fix: Replace unprintable characters
* Update: screenshot.png
* Fixed: Prefix pre_set_page_setting_ hook and get_catmandu_pro_sidebar_layout properly
* Update: Update sidr jquery
* Fix: Fix header 1 layout design issue of top header
* Fix: Fix custom.js issue

= 1.0.0 =
* Released: May 22, 2020

== Copyright ==

Catmandu WordPress Theme, Copyright 2020 CodeManas
Catmandu is distributed under the terms of the GNU GPL.

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

== Resources ==

Catmandu bundles the following third-party resources:

jquery.counterup.js 1.0
https://github.com/bfintal/Counter-Up
Copyright 2013, Benjamin Intal http://gambit.ph @bfintal
Released under the GPL v2 License

jQuery Waypoints - v2.0.3
Copyright (c) 2011-2013 Caleb Troughton
Dual licensed under the MIT license and GPL license.
https://github.com/imakewebthings/jquery-waypoints/blob/master/licenses.txt

jQuery Cycle2; version: 2.1.6 build: 20141007
http://jquery.malsup.com/cycle2/
Copyright (c) 2014 M. Alsup; Dual licensed: MIT/GPL

fakeLoader.js
https://github.com/joaopereirawd/fakeLoader.js
The MIT License (MIT)
Copyright (c) 2014 João Pereira

Font Awesome Free 5.2.0 by @fontawesome - https://fontawesome.com
License - https://fontawesome.com/license (Icons: CC BY 4.0, Fonts: SIL OFL 1.1, Code: MIT License)

The Elegant Icon Font
https://www.elegantthemes.com/blog/resources/elegant-icon-font
Licensed under the GPL 2.0 and MIT

imagesLoaded PACKAGED v4.1.4
https://imagesloaded.desandro.com/
MIT License

Isotope v1.5.25
http://isotope.metafizzy.co
http://isotope.metafizzy.co/docs/license.html
Non-commercial use is licensed under the MIT License
Copyright 2013 Metafizzy

prettyPhoto
Author: Stephane Caron (http://www.no-margin-for-errors.com)
https://github.com/scaron/prettyphoto

Sidr
Released under the MIT License http://opensource.org/licenses/MIT
Copyright © 2013-2017 Alberto Varela (http://www.berriart.com)
https://github.com/artberri/sidr

slick.js
Version: 1.6.0
Author: Ken Wheeler
Repo: http://github.com/kenwheeler/slick
Copyright (c) 2017 Ken Wheeler
Licensed under the MIT license.

WOW
https://github.com/graingert/WOW/
Originally Developed by Matthieu Aussaguel, mynameismatthieu.com Forked to remain under the MIT license by Thomas Grainger, https://graingert.co.uk

animate.css
https://daneden.github.io/animate.css/
licensed under the MIT license

trt-customizer-pro
License: GPL-2.0+
Source: https://github.com/justintadlock/trt-customizer-pro

TGMPA
License: GPL-2.0+
Source: http://tgmpluginactivation.com/

Kirki
License: MIT
Source: https://github.com/aristath/kirki

Bundled image and image in screenshot
* custom-header.jpg - https://stocksnap.io/photo/news-newspaper-OJXHDWMWV9 StockSnap's CC0 License
